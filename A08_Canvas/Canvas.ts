namespace Canvas {
  ...
  let rad: number = Math.floor(Math.random() * (50 - 3)) + 3;
  let x: number = Math.floor(Math.random() * (50 - 3)) + 3;
  ...
  let radGradient: CanvasGradient = crc.createRadialGradient(0, 0, 0, 0, 0, x);
